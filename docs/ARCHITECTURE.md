...
- **누굴 위해** — 구닥다리 홈페이지를 운영 중인 **숙박업 전체**(호텔·모텔·리조트·펜션·
  게스트하우스) 사장님. 모바일도 안 되고, 외국어도 안 되고, 예약 버튼은 전화번호뿐인
  그 사이트의 주인.
...
**제품 컨셉 한 줄 (확정): "도메인만 있으면, 몇 시간 안에 홈페이지 완성."**
URL 입력(1분) → 생성·미리보기(1분) → 사장님 검수(요금·사진 확인) → DNS 레코드 1개
변경 → 라이브. 구 주소들은 301로 자동 연결되어 검색 순위까지 함께 이사한다.
...
### K — 숙박업 전체로 확장: property_type 📌 확정 (스키마 반영: 0009)
`hotels.property_type` (hotel/motel/resort/pension/guesthouse) 하나가 위저드 카피 톤,
요금 기본값(모텔 6만~ / 리조트 18만~), 펜션 독채(객실당 1실) 구성, schema.org 타입
(Hotel/Motel/Resort/BedAndBreakfast)을 결정한다. 위저드가 옛 사이트 텍스트에서 자동 감지.
**모텔 대실(시간제)은 의도적 로드맵**: 재고 원장에 슬롯 차원이 추가되는 대공사라
`day_use_slots` 테이블 + 전용 RPC 설계 방향만 0009에 명시. 숙박 예약은 모텔도 현 구조로 동작.

### L-1 — 콘텐츠 2필러: 글 공장이 아니라 콘텐츠 📌 확정 (스키마: 0010)
글의 종류가 곧 품질 규칙이다:

| 필러 | 원료 | 그라운딩 규칙 |
|---|---|---|
| `local_guide` 지역 정보성 | **사장님 메모** | 장소명·거리·시간 등 구체 사실은 메모에 있는 것만. AI는 지어내지 못함 |
| `hotel_guide` 숙소 정보성 | **등록된 실제 데이터** (객실·체크인 시간·시설) | 사실 블록 외 발명 금지, FAQ 자동 포함(리치결과 노림) |

+ 추천 주제 엔진(업종 × 계절)이 콘솔에 칩으로 노출 — 빈 입력창 대신 에디토리얼 캘린더.
이 구조가 E-E-A-T(경험 기반 콘텐츠)와 스케일드 콘텐츠 남용 페널티 양쪽에 대한 답이다.

### L — 마케팅 자동화: 블로그가 실장착됨 📌 확정
- posts 렌더링: `/news`(목록) + `/news/{slug}`(상세) — 본문은 페이지와 동일한 섹션 모델,
  sitemap·hreflang 포함, 게시물이 있을 때만 네비에 "소식" 노출
- **AI 글 작성**: `POST /api/posts/generate` {topic, kind} → Claude가 테넌트 locale 전체로
  작성(zod 검증 통과분만 게시). 데모 = 즉시 게시, 운영(Supabase) = **draft 저장 후 검수**
  — 자동 생성 콘텐츠는 검수 없이 라이브되지 않는다는 규칙
- 위저드가 "홈페이지 새단장" 공지 1호를 자동 생성 — 블로그가 첫날부터 비어있지 않음
- 다음 단계: 이벤트/크론 구독 캠페인(시즌·예약 데이터 기반 자동 초안), 네이버 블로그는
  발행 API가 없으므로 자체 블로그 + RSS 발행으로 (0008 연결 레이어의 역방향)
